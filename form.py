...
def getFunctionPopulateForm(pluginName):
    ...
    getPopulateForm = lambda plugin: plugin.alertMB.mb.populateForm # class_instance.py: _init_()
    plugins = {}
...
def loadForm(dialog, layer, feature):
    ...
    widgets = {
        'alert_id': dialog.findChild( QLabel, 'alert_id'),
        'validation_date': dialog.findChild( QLabel, 'validation_date'),
        'geom_area_ha': dialog.findChild( QLabel, 'geom_area_ha'),
        'before_image': dialog.findChild( QLabel, 'before_image'),
        'after_image': dialog.findChild( QLabel, 'after_image'),
        'before_thumbnail': dialog.findChild( QLabel, 'before_thumbnail'),
        'after_thumbnail': dialog.findChild( QLabel, 'after_thumbnail'),
        'message_status': dialog.findChild( QLabel, 'message_status')
    }
    if populateForm is None:
        r = getFunctionPopulateForm('alertmb') # plugin directory
        if not r['isOk']:
            widgets['message_status'].setText( r['message'] )
            return
        populateForm = r['function']
    if feature.fieldNameIndex('alert_id') == -1:
        return
